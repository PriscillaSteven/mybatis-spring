...
public class ClassPathMapperScanner extends ClassPathBeanDefinitionScanner {
  ...
  private void processBeanDefinitions(Set<BeanDefinitionHolder> beanDefinitions) {
    ...
    for (BeanDefinitionHolder holder : beanDefinitions) {
       ...
       * 该行代码原本我们的bean定义假设是一个com.tuling.Mapper.Usermapper类型的class 但是我们知道接口是不能被实例化的，
       * 所以我们在这里进行替换把class对象替换成MapperFactoryBean类型的,因为这个类型是一个类(而且实现了我们) 的FactoryBean接口
       * 这个接口会提供一个getObject()方法来返回我们的UserMapper接口的 代理对象.但是我们知道基于jdk代理对象 是面向我们的接口代理,那么我们需要把原始的接口
       * 传入给MapperFactoryBean类中,然后才能在MapperFactoryBean 的getObject类型返回我们的代理对象
       * definition.getConstructorArgumentValues().addGenericArgumentValue(beanClassName); 在spring
       * ioc容器实例化我们的UserMapper的时候(MapperFactoryBean) 会调用构造器函数 把beanClassName传入 到MapperFactoryBean的mapperInterface
       * 那么spring ioc在实例化我们的UserMapper对象的时候 实际就是初始化我们的MapperFactoryBean对象
       */
